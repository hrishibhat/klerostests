...
describe("Demo pre-alpha1", function () {
  ...
  it("Demo", async () => {
    ...
    console.log(foreignGateway.address);
    let receiver = await ethers.utils.defaultAbiCoder.decode(["address", "bytes" ], messageData);
    ...
    const balance = await fastBridgeReceiver.provider.getBalance(fastBridgeReceiver.address);
    ...
    console.log(bridger.address);
    console.log(bridgerBalance.toBigInt());
    const challengeDuration = await fastBridgeReceiver.challengeDuration();
    console.log(challengeDuration.toNumber());
